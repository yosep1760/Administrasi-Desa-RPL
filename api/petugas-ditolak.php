...
$query = "SELECT ps.*, u.nama_lengkap AS nama_warga, js.nama_surat FROM Pengajuan_Surat ps JOIN Users u ON ps.id_user = u.id_user JOIN Jenis_Surat js ON ps.id_jenis = js.id_jenis WHERE ps.status = 'ditolak' ORDER BY ps.tanggal_pengajuan DESC";
                  ...
                  <?php $no = 1; while($row = $data_surat->fetch_assoc()): ?>
                    ...
                    <tr>
                      ...
                      <td>
                          ...
                          <span style="font-size:0.85rem; color:#4b5563;"><?= htmlspecialchars($alasan) ?></span>
                      </td>
                    </tr>
                  <?php endwhile; ?>
              <?php else: ?>
                  <tr><td colspan="5" style="text-align:center; padding: 20px;">Belum ada riwayat surat ditolak.</td></tr>
              <?php endif; ?>
            </tbody>
          </table>
        </div>
